...
const useResponsiveDimension = (
  ...
  const [val, setVal] = useState<GradualBlurProps[keyof GradualBlurProps]>(
    config[key]
  );
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v: GradualBlurProps[keyof GradualBlurProps] = config[key];
      ...
      if (w <= 480 && config[mobileKey]) v = config[mobileKey];
      else if (w <= 768 && config[tabletKey]) v = config[tabletKey];
      else if (w <= 1024 && config[desktopKey]) v = config[desktopKey];
      setVal(v);
    };
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      ...
      if (
        ...
        baseStyle.width = responsiveHeight;
      }
